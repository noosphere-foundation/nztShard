...
class ENode(QObject):
    ...
    def sendElasticClientsInfo(self):
        ...
        for i in range(0, 500):
            ...
            eljson = {"_index": "clients", "_type": "clients", "pipeline": "geoip",
                      "_source": {"@dtime": int(time.time()), "ip": ip, "public_key": NooTree.hash(ip),
    ...
    def eLog(self):
        while True:
            bchainHeight = self.mongo.noochain.find().count()

            if self.lastLogBlock == bchainHeight: #FULL LOG COMPLETE!
                break

            blocks = list(self.mongo.noochain.find({"BHEIGHT": int(self.lastLogBlock)}))
            #blocks = self.redis.zrange("NOOCHAIN", self.lastLogBlock, self.lastLogBlock)
            self.lastLogBlock += 1
            self.__eLog(blocks)
    ...
    def __eLog(self, blist):
        for block in blist:
            ...
            if jblock["TT"] == "BL":
                self.elka.elasticBlock(int(time.time()), jblock["SENDER"], int(jblock["TCOUNT"]), jblock["SIGNATURE"], NooTree.hash(block), jblock["BHEIGHT"])
                    ...
                    if tran["TT"] == "ST":
                        ...
                        eljson = {"_index": "transactions-" + self.lastDate, "_type": "transactions-" + self.lastDate,
                                  "_source": {  "@dtime": int(tran["TST"]),
                                                ...
                                                "hash": NooTree.hash(json.dumps(tran, separators=(',', ':')))
                                    }
                                  }
