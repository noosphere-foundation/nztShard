            ...
            for tran in txmas:
                ...
                if jtran["TT"] == "AT":
                    ...
                    if tempBalanceMemmory is None:
                        tempBalanceMemmory = {}
                        tempBalance = self.redis.zscore("BALANCE:NZT", currentSender)
                    else:
                        tempBalance = tempBalanceMemmory.get(currentSender)
    ...
    def balanceMainWorkRedis(self, bchain:list):
        for jblock in bchain:
            ...
            if jblock['TT'] == "BL":
                ...
                blockSenderBalance = self.redis.zscore("BALANCE:NZT", jblock['SENDER'])
                blockSenderBalance += self.blockReward
                print("BALANCE_BHEIGHT", jblock['BHEIGHT'])
                print("FREEZZE:", self.getFreezeTokens(jblock['SENDER']))
                print("ADDING BALANCE NZT TO VALIDATOR:", blockSenderBalance - self.getFreezeTokens(jblock['SENDER']), jblock['SENDER'])
                self.redis.zadd("BALANCE:NZT", blockSenderBalance, jblock['SENDER'])
...
if __name__ == '__main__':
    ...
    privateKey = "125466fda87a6c88df7222eec063a720958985e39d9e6eff5c76047e68d32c67"
