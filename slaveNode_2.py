...
from nooCrypto.signChecker import SignChecker

from netTools import NetEngine
from jsonMaker import JsonPackets

from datetime import datetime, date, time
import json, sys, os, redis, pymongo
...
class slaveWorker(QThread):
    ...
    def handler(self, address: str, packet: str):
        ...
        print("RECIVE PACKET: ", jpacket["TT"], "FROM:", address)
        ...
        if jpacket["TT"] == "CBRH":
                ...
                if jpacket["SENDER"] == self.stemPublicKey: #IF SENDER IS MASTER NODE
                    ...
                    if NooTree.hash(json.dumps(JTPCB, separators=(',', ':'))) != hash:
                        print("BAD HASH!!! FROM CBRH:", NooTree.hash(self.precommitedBlock), "AND", hash)
                    ...
                    for tran in block["TRANSACTIONS"]:
                        ...
                        if tran["TT"] == "ET":
                            ...
                            sign = tran["SIGNATURE"]
                            self.redis.zadd("COMPLETE TRANSACTIONS", int(block["BHEIGHT"]), NooTree.hash(sign))
                            self.redis.set("TRANSACTIONS:" + NooTree.hash(sign), stran)
        ...
        if jpacket["TT"] == "BL":
            ...
            if okSign:  # Good block!
                ...
                self.precommitedBlock = precommitedBlock
                print("SEND MY SIGN to ->", address)
                address = '127.0.0.3'
                self.sendPacket.emit(address, JsonPackets.createSignaturePrecommitedBlock(self.cryptor, precommitedBlock, NooTree.hash(precommitedBlock)), 1)


            print("OK SIGN:", okSign)
        ...
        if jpacket["TT"] == "CT":
            ...
            for tran in txmas:
                ...
                if jtran["TT"] == "ET":
                    ...
                    if tempSBalance is None:
                        ...
                        self.redis.zadd("FAILED TRANSACTIONS", 1, NooTree.hash(jtran["SIGNATURE"]))
                    ...
                    if totalSBalance < (float(stc) + self.getAnyFee(stt)):
                        self.redis.zrem("RAW TRANSACTIONS", tran)
                        self.redis.zadd("FAILED TRANSACTIONS", 2, NooTree.hash(jtran["SIGNATURE"]))
                ...
                if jtran["TT"] == "AT":
                    ...
                    # CHECK DOUBLE MONEY SEND
                    tempBalanceMemmory = tempDictBalanceMemmory.get("VNC")
                    if tempBalanceMemmory is None:
                        tempBalanceMemmory = {}
                        tempBalance = self.redis.zscore("BALANCE:VNC", currentSender)
                    else:
                        tempBalance = tempBalanceMemmory.get(currentSender)
                    ...
                    tempDictBalanceMemmory.update({"VNC": tempBalanceMemmory})
                    decodeTxmas.append(tran)
            ...
            self.signatureMemory.append(self.cryptor.signMessage(precommitBlock))

            signNeedCount = len(set(self.signatureMemory))

            print("1111:", signNeedCount)
            print("2222:", self.nodesCount)
            print("3333:", int((2 / 3) * self.nodesCount))


            if signNeedCount >= int((2 / 3) * self.nodesCount): # !!!ONLY WORK IN TESTNET WITH ONE NODE!!!
                print("SIGN COMPLETE")
                #self.transactionMemory = self.transactionMemory - self.transactionsInPBlock  # ДОБАВИТЬ ПОИСК И УДАЛЕНИЕ
                # CLEAR TRAN LIST #
    ...
    def getFreezeTokens(self, wallet, token_type = "VNC"):
    ...
    def balanceMainWorkRedis(self, bchain:list):
        for jblock in bchain:
            ...
            if jblock['TT'] == "BL":
                ...
                if self.redis.zscore("BALANCE:VNC", jblock['SENDER']) is None:
                    self.redis.zadd("BALANCE:VNC", 0, jblock['SENDER'])

                blockSenderBalance = self.redis.zscore("BALANCE:VNC", jblock['SENDER'])
                blockSenderBalance += self.blockReward
                print("BALANCE_BHEIGHT", jblock['BHEIGHT'])
                print("FREEZZE:", self.getFreezeTokens(jblock['SENDER']))
                print("ADDING BALANCE VNC TO VALIDATOR:", blockSenderBalance - self.getFreezeTokens(jblock['SENDER']), jblock['SENDER'])
                self.redis.zadd("BALANCE:VNC", blockSenderBalance, jblock['SENDER'])
    ...
    def checkBalance(self, updateLen = -1):
        ...
        if updateLen == -1:
            ...
            bchain = list(self.mongo.noochain.find({"BHEIGHT": {"$gte": int(self.lastCheckBlockBalance)}}))
            print("BCH", bchain)
            ...
            while step < bheight:
                ...
                bchain = list(self.mongo.noochain.find([{"BHEIGHT":{"$gt":step}},{"BHEIGHT":{"$lt":nextStep}}]))
                step = nextStep
                self.balanceMainWorkRedis(bchain)
    ...
    def run(self):
        while True:
            ...
            if self.signChecker.checkTran(packet):
...
if __name__ == '__main__':
    ...
    privateKey = "771a158e709d0569162f4844578a362dbac44273812a392a64f412067f026a0b"
    ...
    slave.start()
    netEngine.runReceiver.emit("127.0.0.2")
    sys.exit(app.exec())
